...
interface CardProps {
  ...
  disabled?: boolean;
  showOwner?: string;
  /** Enable scale-up zoom effect on hover */
  zoomOnHover?: boolean;
}
...
export function Card({ card, onClick, selected, disabled, showOwner, zoomOnHover }: CardProps) {
  return (
    <button
      ...
      className={`relative group rounded-xl overflow-hidden transition-all duration-300 origin-center ${
        zoomOnHover
          ? "hover:scale-150 hover:z-40 hover:shadow-2xl hover:shadow-black/60 hover:ring-2 hover:ring-white/20"
          : ""
      } ${
        onClick && !disabled
